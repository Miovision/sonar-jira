...
public class LinkFunction implements Function, ServerExtension {
	...
	BasicIssue issue;
	try {
	    issue = jiraIssueCreator.createIssue(context.issue(),
		    context.projectSettings());
	} catch (RemoteException e) {
	    throw new IllegalStateException(
		    "Impossible to create an issue on JIRA. A problem occured with the remote server: "
			    + e.getMessage(), e);
	}

	createComment(issue, context);
	// and add the property
	context.setAttribute(JiraConstants.SONAR_ISSUE_DATA_PROPERTY_KEY,
		issue.getKey());
    }
	...
	if (!settings.hasKey(property) && !settings.hasDefaultValue(property)) {
	    throw new IllegalStateException(
		    "The JIRA property \""
			    + property
			    + "\" must be defined before you can use the \"Link to Jira\" button");
	}
    }
	...
	message.append(context.projectSettings().getString(
		JiraConstants.SERVER_URL_PROPERTY));
