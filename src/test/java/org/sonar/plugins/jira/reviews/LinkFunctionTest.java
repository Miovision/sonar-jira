...
public class LinkFunctionTest {
	...
	remoteIssue = new BasicIssue(null, "FOO-15", (long) 0);
	when(jiraIssueCreator.createIssue(sonarIssue, settings)).thenReturn(
		remoteIssue);
	...
	verify(context).setAttribute(
		JiraConstants.SONAR_ISSUE_DATA_PROPERTY_KEY, "FOO-15");
    }

    @Test
    public void should_fail_execute_if_remote_problem() throws Exception {
	when(jiraIssueCreator.createIssue(sonarIssue, settings)).thenThrow(
		new RemoteException("Server Error"));
	...
	verify(context)
		.addComment(
			"Issue linked to JIRA issue: http://my.jira.server/browse/FOO-15");
    }
	...
	settings.setProperty(JiraConstants.SERVER_URL_PROPERTY,
		"http://my.jira.server");
	settings.setProperty(JiraConstants.SOAP_BASE_URL_PROPERTY,
